...
class PortfolioVisualizer:
    def __init__(self):
        ...
        # Initialize timestamp for unique filenames
        self.timestamp = None
    ...
    def set_file_prefix(self, transaction_file: str):
        """Set the base filename and timestamp for generating unique image filenames"""
        self.base_filename = os.path.splitext(os.path.basename(transaction_file))[0]
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    def get_image_filename(self, plot_type: str) -> str:
        """Generate a unique filename for the plot"""
        if self.base_filename is None:
            self.base_filename = "portfolio"
        if self.timestamp is None:
            self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        return os.path.join(self.images_dir, f"{self.base_filename}_{plot_type}_{self.timestamp}.png")
    ...
    def plot_equity_curve(self, portfolio_values: pd.Series, title: str = "Portfolio Equity Curve"):
        ...
        # Save the plot
        filename = self.get_image_filename("equity_curve")
    ...
    def plot_drawdown(self, portfolio_values: pd.Series, title: str = "Portfolio Drawdown Analysis"):
        ...
        # Save the plot
        filename = self.get_image_filename("drawdown")
        plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
    ...
    def create_monthly_returns_heatmap(self, portfolio_values: pd.Series, title: str = "Monthly Returns Heatmap"):
        ...
        # Save the plot
        filename = self.get_image_filename("monthly_returns")
        plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
...
def main():
    ...
    visualizer.set_file_prefix("sample_transaction_file.csv")
